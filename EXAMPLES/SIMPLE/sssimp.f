...
c
      ...
      parameter       (maxn=1256, maxnev=110, maxncv=525,
     $                 ldv=maxn )
c
c     %--------------%
c     | Local Arrays |
...
Cc
Cc ------------------------------------------------------------------
Cc     matrix vector subroutine
Cc
Cc     The matrix used is the 2 dimensional discrete Laplacian on unit
Cc     square with zero Dirichlet boundary condition.
Cc
...
C      integer           nx, j, lo, n2
C      Real
C     &                  v(nx*nx), w(nx*nx), one, h2
C      parameter         ( one = 1.0E+0 )
Cc
C      call tv(nx,v(1),w(1))
C      call saxpy(nx, -one, v(nx+1), 1, w(1), 1)
Cc
C      do 10 j = 2, nx-1
C         lo = (j-1)*nx
C         call tv(nx, v(lo+1), w(lo+1))
C         call saxpy(nx, -one, v(lo-nx+1), 1, w(lo+1), 1)
C         call saxpy(nx, -one, v(lo+nx+1), 1, w(lo+1), 1)
C  10  continue
Cc
C      lo = (nx-1)*nx
...
C      call saxpy(nx, -one, v(lo-nx+1), 1, w(lo+1), 1)
Cc
Cc     Scale the vector w by (1/h^2), where h is the mesh size
Cc
C      n2 = nx*nx
C      h2 = one / real((nx+1)*(nx+1))
C      call sscal(n2, one/h2, w, 1)
C      return
C      end
c
c-------------------------------------------------------------------
