...
c
      ...
      parameter       (maxn=111256, maxnev=10, maxncv=1565,
     $                 ldv=maxn )
c
c     %--------------%
c     | Local Arrays |
...
C      call daxpy(nx, -one, v(nx+1), 1, w(1), 1)
Cc
C      do 10 j = 2, nx-1
C         lo = (j-1)*nx
C         call tv(nx, v(lo+1), w(lo+1))
C         call daxpy(nx, -one, v(lo-nx+1), 1, w(lo+1), 1)
C         call daxpy(nx, -one, v(lo+nx+1), 1, w(lo+1), 1)
C  10  continue
Cc
C      lo = (nx-1)*nx
...
Cc
C      n2 = nx*nx
C      h2 = one / dble((nx+1)*(nx+1))
C      call dscal(n2, one/h2, w, 1)
C      return
C      end
Cc
Cc-------------------------------------------------------------------
